...
| for i in arr.iter_mut()	| Ссылка (&mut T)	            | ✅         | ✅                             |

Теперь все варианты итерации учтены. Спасибо за внимание к деталям!
...
2.2 Итерация по векторам в Rust

В Rust есть несколько способов итерироваться по вектору (`Vec<T>`), и каждый из них имеет свои особенности. 

## 2.2.1. Итерация по значению (`for i in vec`)
...
- Элементы **перемещаются** из `vec` внутрь цикла.
- После итерации вектор **становится недоступным**.
- Если `T` реализует `Copy`, элементы будут **скопированы** при передаче в цикл, но сам `Vec<T>` все равно уничтожится.

✅ **Используйте `for i in vec`, если вам нужно забрать элементы из вектора, так как сам вектор станет недоступен после итерации.**

---

## 2.2.2. Итерация по ссылке (`for i in &vec`)
...
- `&vec` передается как ссылка (`&[T]`), благодаря чему элементы не перемещаются.
- Итерация происходит **по ссылкам (`&T`)**.
- Оригинальный вектор остается доступным после цикла.

---

## 2.2.3. Итерация через `.iter()`
...
- `.iter()` создаёт **итератор по ссылкам (`&T`)**.
- Используется для функционального программирования, например:
...
## 2.2.4. Итерация через `.into_iter()`
...
- `.into_iter()` **перемещает** элементы из вектора, аналогично `for i in vec`.
- В Rust 1.53+ `.into_iter()` для `Vec<T>` ведёт себя так же, как `for i in vec`, поэтому чаще используется `.iter()` или `.iter_mut()`.
- После итерации вектор становится недоступным.

✅ **В новых версиях Rust лучше использовать `.iter()` или `.iter_mut()`, если не нужно перемещать элементы.**

---

## 2.2.5. Итерация через `.iter_mut()` (изменение элементов)
...
- `.iter_mut()` создаёт **итератор по изменяемым ссылкам (`&mut T`)**.
- Позволяет **изменять элементы вектора** внутри цикла.
- Оригинальный вектор остается доступным после итерации.

---

## **Вывод**

| Способ                      | Владение                     | Изменение | Оригинальный вектор доступен? |
| --------------------------- | ---------------------------- | --------- | ----------------------------- |
| `for i in vec`              | **Перемещает** элементы      | ❌         | ❌                             |
| `for i in &vec`             | **Ссылка (`&T`)**            | ❌         | ✅                             |
| `for i in vec.iter()`       | **Ссылка (`&T`)**            | ❌         | ✅                             |
| `for i in vec.into_iter()`  | **Перемещает** элементы      | ❌         | ❌                             |
| `for i in vec.iter_mut()`   | **Ссылка (`&mut T`)**        | ✅         | ✅                             |

💡 **Что выбрать?**

- **`for i in vec`** — если нужно забрать элементы из вектора.
- **`for i in &vec` или `.iter()`** — если элементы должны остаться доступными.
- **`.iter_mut()`** — если нужно изменять элементы.
- **`.into_iter()`** — редко используется, так как ведёт себя как `for i in vec`.
